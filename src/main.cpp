...
void loop()
{
    ...
    if (!keyTriggered && enterTriggered && currentExitState && !exitTriggered)
    {
    ...
    if (!keyTriggered && exitTriggered && currentEnterState && !enterTriggered)
    {
        enterTriggered = true; // Xác nhận cảm biến vào đã kích hoạt sau cảm biến ra
        if (passengerCount > 0)
            passengerCount--; // Giảm số lượng người nếu > 0
